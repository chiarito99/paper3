...
global flag,err1,err2,cons,f1,f2,vleader
class LeaderUAV:
...
if __name__ == "__main__":
    ...
    x_end = 80
    y_end = 80
    K = [[59, -40],[-10,-40],[-20, -85], [-63, 24], [-31, 61], [52, 26]]
    # M = [ [59, -40],[40, -20], [-10,-40],  [-63, 24],[-31, 56] , [52, 26]]
    # K = [[45.71, -52.71], [-23.09, -50.92], [-55.21, -24.46], [-41.61, 66.07], [36.29, 26.69]]
    # K = [[40, 10],[-15.18, -75.32], [-64.54, -37.54], [-13.62, 49.98], [66.52, 14.37]]
    # polygon1,polygon2 = plan(M)
    # print(polygon1)
    # print(polygon2)
    ...
    ox2 ,oy2 = zip(*K2)
    print(K2)
    b_path = planning(4.2,1,ox2,oy2,[],0)
    for i in range(len(b_path[0])):
        path2.append([b_path[0][i],b_path[1][i]])
    path2.append(K2[0])
    map2 = Env1([x_start,y_start],[x_end,y_end],4.2,path2)
    ...
    for i in range(1, 3659):
        ...
        if flag_point1 == 1:
            if var < len(map2.traj[0]):
                ...
                if check_wp-1 == wp_break:
                    wp_break = check_wp-1
                    flag_point1 = 0
                    d_path = planning((resolution+8)/2,1,ox1,oy1,path1[wp_break],wp_break-1)
                    del path1[wp_break-1:len(path1)]
                    for i in range(len(d_path[0])):
                        path1.append([d_path[0][i],d_path[1][i]])
                    path1.append([x_end,y_end])
                    map1 = Env1([x_start,y_start],[x_end,y_end],(resolution+8)/2,path1)
                    len_map1 = len(map1.traj[0])
                    casee = 2
        else:
            f1vel = follower1.control_signal(ref1,flag_point1)
        ...
        if wp_break+2 == check_wp and check_wp_break==1:
            print("here")
            e_path = planning(resolution,1,ox1,oy1,path1[wp_break+2],wp_break+1)
            del path1[wp_break+1:len(path1)]
        ...
        if err1+err2- 2*disLF > 0.7 and err1+err2- 2*disLF < 1.5:
            flag = 1
        elif (abs(leader.heading - follower1.heading) >np.pi/2 and abs(leader.heading - follower2.heading) >np.pi/2) :
            flag = 2
        else:
            flag=0 
    ...
    plot.plot_animation(leader_p,follower1_p,follower2_p,ox, oy,x_start,y_start,x_end,y_end,length,width)
    plt.show()
